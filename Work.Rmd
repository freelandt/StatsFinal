...
VideoGames2$EU_Sales2 <- round(VideoGames2$EU_Sales)
...
ggplot(filter(edaData2, Region %in% c("EU_Sales", "NA_Sales")), aes(x = Genre, y = Percent, fill = Region)) + geom_bar(stat = "identity", position = "dodge")
...
VideoGames5 <- VideoGames2 %>% filter(JP_Sales > 0)
...
jp.sales <- glmer((JP_Sales*100)~Year2 + Genre + Critic_Score2+User_Score+Rating2 + (1|Publisher2) + (1|Platform), data = VideoGames5, family = poisson)

#After deliberation we believe that it would be best/make most sense to use a hurdle model. 

stargazer(na.sales2, eu.sales, jp.sales, type='text')
```
...
eu.sales <- lmer(log(EU_Sales+.001)~Year_of_Release+Genre+Critic_Score+User_Score+Rating2+(1|Platform)+(1|Publisher2), data = VideoGames2)
summary(eu.sales)

plot(resid(eu.sales)~log(VideoGames2$EU_Sales+.001))

ggplot(VideoGames2, aes(x = NA_Sales))+geom_histogram(binwidth = .4)
